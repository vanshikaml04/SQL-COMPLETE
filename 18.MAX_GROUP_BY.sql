-- MAX: returns the largest value in a column.
-- Example: SELECT MAX(Salary) FROM Employees;
-- Gets the highest salary from Employees table.
-- Often used with GROUP BY to find max per group.
-- Example: SELECT Department, MAX(Salary) FROM Employees GROUP BY Department;
-- Gives highest salary in each Department.

-- GROUP BY: groups rows that have the same values into summary rows.
-- Example: SELECT Department, COUNT(*) FROM Employees GROUP BY Department;
-- Counts number of employees in each Department.
-- Often used with aggregate functions: COUNT, SUM, AVG, MAX, MIN.
-- GROUP BY must include all non-aggregated columns in SELECT.
-- Example: SELECT Department, AVG(Salary) FROM Employees GROUP BY Department;
-- Gives average salary per Department.


create database [Sales database3]

use [sales database3]

-- Creating a table with more columns, including a varchar column
CREATE TABLE dbo.Sales (
    ProductID INT,
    SaleDate DATE,
    Quantity INT,
    TotalAmount DECIMAL(10, 2),
    CustomerID INT,
    StoreID INT,
    SalespersonID INT,
    PaymentMethod VARCHAR(50)  -- New varchar column for payment method
);

-- Inserting more sample data (10 records), including NULL values
INSERT INTO dbo.Sales (ProductID, SaleDate, Quantity, TotalAmount, CustomerID, StoreID, SalespersonID, PaymentMethod)
VALUES 
(1, '2023-08-01', 10, 200.00, 101, 1, 201, 'Credit Card'),
(2, '2023-08-01', 5, 150.00, 102, 1, 202, 'Cash'),
(1, '2023-08-02', 8, 160.00, 103, 2, 203, 'Credit Card'),
(2, '2023-08-02', 7, 210.00, 104, 2, 204, 'Cash'),
(1, '2023-08-03', 6, 120.00, 105, 1, 201, NULL),              -- NULL value in PaymentMethod
(3, '2023-08-04', 12, 300.00, 106, 3, 205, 'Credit Card'),
(1, '2023-08-04', 5, 100.00, 107, 3, 206, 'Debit Card'),
(2, '2023-08-05', 9, 270.00, 108, 1, 202, NULL),              -- NULL value in PaymentMethod
(3, '2023-08-05', 15, 375.00, 109, 3, 207, 'Cash'),
(1, '2023-08-06', 7, 140.00, 110, 2, 203, 'Credit Card');


select * from [dbo].[Sales]

select  MAX(TotalAmount) [max amount,max date] from [dbo].[Sales]

select max(saledate) [max date] from [dbo].[Sales]

select max(paymentmethod ) [pay] from [dbo].[Sales] -- last alphabetically

select productId, max(totalamount) [max amount] from [dbo].[Sales]
group by ProductID

select saledate, max(totalamount) [max amount] from [dbo].[Sales]
group by saledate
